...
import com.generic.base.World.ShadedTrianglesModel;
...
public class Jogl {
   ...
   private static class Renderer3D implements GLEventListener,                                       /* jogamp   */
      ...
      private void renderGL (GL3 gl) {         
         ...
         for (Graphics3D.Shader.Command command : graphics3D.commands) {
            ...
            if (command instanceof Graphics3D.Shader.Variable.Binding) {
               ...
               if (command instanceof Graphics3D.Shader.Variable.VertexBuffer.Binding) {
                  Graphics3D.Shader.Variable.VertexBuffer.Binding b = (Graphics3D.Shader.Variable.VertexBuffer.Binding) command;
                  if (b.variable == Graphics3D.Shader.POSITIONS) {
                     positionsBufferId = b.vertexBuffer;
                  }
                  if (b.variable == Graphics3D.Shader.NORMALS) {
                     normalsBufferId = b.vertexBuffer;
                  }
                  if (b.variable == Graphics3D.Shader.BARYCOORDS) {
                     baryBufferId = b.vertexBuffer;
                  }
               }
            }

            if (command instanceof Graphics3D.Shader.Command.Execute) {
               Graphics3D.Shader.Command.Execute b = (Graphics3D.Shader.Command.Execute) command;
      ...
      public void mouseMoved(MouseEvent e) {
         if (mouseListener != null) mouseListener.mouseHover(Image.Position.of(e.getX(), e.getY()));
      }

      public void mousePressed(MouseEvent e) {
         if (mouseListener != null) mouseListener.mouseDown(Image.Position.of(e.getX(), e.getY()));
      }
      public void mouseDragged(MouseEvent e) {
         if (mouseListener != null) mouseListener.mouseDrag(Image.Position.of(e.getX(), e.getY()));
      }
      public void mouseReleased(MouseEvent e) {
         if (mouseListener != null) mouseListener.mouseUp();
      }
